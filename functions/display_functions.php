...
function drop_downs_selected($table_name, $id_value, $name_value, $selected_id, $order_by_field = false, $where_criteria = false, $max_length = 256) {//start of the drop downs function
    $link = mysqli_open();
    $queryString = "SELECT * FROM " . $table_name . ($where_criteria == false ? "" : " WHERE " . $where_criteria) . ($order_by_field == false ? "" : " ORDER BY " . $order_by_field);
    $query = mysqli_query($link, $queryString);
    while ($row = mysqli_fetch_assoc($query)) {
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        if (in_array($id,$selected_id)) {
            echo "<label class='checkbox inline'>
                <input type='checkbox' value = '" . $id . "' checked name='$chk_name'>{$name}
                </label>";
        } else {
            echo "<label class='checkbox inline'>
                <input type='checkbox' value = '" . $id . "' name='$chk_name'>{$name}
                </label>";
        }
    }
    mysqli_close($link);
